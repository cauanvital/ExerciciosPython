...
#           de um jogador e o total de votos. A função calculará o percentual e retornará o valor calculado. Abaixo
#           segue uma tela de exemplo. O disposição das informações deve ser o mais próxima possível ao exemplo. Os
#           dados são fictícios e podem mudar a cada execução do programa. Ao final, o programa deve ainda gravar os
...
while True:
    voto = int(input("Número do jogador (\"0\" para finalizar): "))
    if voto == 0: break
...
qtdVotos = sum(contagem.values())

with open("EL-018.txt", "w") as arquivo:
    arquivo.write("Resultado da votação:\n\n"
                  f"Foram computados {qtdVotos} votos.\n\n"
                  f"Jogador | Quantidade de Votos | Porcentagem")

jogadorMais = None
for i in contagem:
    if jogadorMais is None:
        jogadorMais = i
    elif contagem[i] > contagem[jogadorMais]:
        jogadorMais = i

    with open("EL-018.txt", "a") as arquivo:
        arquivo.write(f"\n{i}{' ' * (8 - len(str(i)))}| "
                      f"{contagem[i]}{' ' * (20 - len(str(contagem[i])))}| "
